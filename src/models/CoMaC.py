...
class CoMaC(baseCoMaC):
    ...
    def cluster_seq(self, X, beta=0.5, g_init=None, max_iter=25, pr = True):
        ...
        if pr: print('*'*25+'Clustering Finished!'+'*'*25)

        return cost, V
    
    
    def cluster_ann(self, X, final_beta=0, step_size=-0.5, max_iter=25, pr = True):
        ...
        if pr : print(f'Starting with beta = 1')
        (cost_temp, V_temp) = self.cluster_seq(X, beta=1, max_iter=max_iter)
        cost[0] = cost_temp
        V_ann[0, :, :] = V_temp

        for i in range(1, len(beta)):
            if pr : print(f'Starting with beta = {beta[i]}')
